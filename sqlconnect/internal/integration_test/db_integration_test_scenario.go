...
func TestDatabaseScenarios(t *testing.T, warehouse string, configJSON json.RawMessage, formatfn func(string) string, opts Options) {
	...
	t.Run("table admin", func(t *testing.T) {
		...
		t.Run("table exists in catalog", func(t *testing.T) {
			...
			tableWithNonexistentCatalog := table
			tableWithNonexistentCatalog.Catalog = "nonexistent"
			exists, err = db.TableExists(ctx, tableWithNonexistentCatalog)
			require.NoError(t, err, "it should be able to check if a table exists in a nonexistent catalog")
			require.False(t, exists, "it should return false for a table in a nonexistent catalog")
		})
